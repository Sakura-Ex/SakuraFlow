...
* **SQLite 持久化**: 使用 `sf_tasks/tasks.db` 进行存储，启动时会自动迁移旧版 `tasks.json` 并生成 `.bak` 备份。
...
1. **创建任务**
    示例：创建一个名为“建造刷铁机”的任务。任务会自动分配一个唯一的ID，后续操作该任务需要使用其ID。
    `!!todo add 建造刷铁机`
2. **查看列表**
    显示当前所有待办任务。
    `!!todo list`
...
3. **追加记录**
    为任务添加详细说明或进度笔记。
    `!!todo note <ID> 需要准备 20 张床` 或者 `!!todo note <ID> 已完成建筑材料的准备`
...
| 动作     | 指令格式                   | 别称  | 说明                     |
|:-------|:------------------|:----|:-----------------------|
| **新建** | `!!todo add <标题>`      | `a` | 创建一个新任务。任务会自动分配一个唯一的ID，后续操作该任务需要使用其ID。   |
| **列表** | `!!todo list`          | `l` | 显示所有**进行中**的任务（含交互按钮）。**已完成**的任务见后文进阶管理。 |
...
以下是从零开始管理一个“刷铁机建造工程”的完整指令流：

### 1. 创建与基础设置
* **创建主任务**：
  `!!todo add 建造高效刷铁机` (假设分配 ID 为 `1`)
* **设置任务等级**：
  `!!todo set 1 tier 5` (将等级设为 5，使其在列表中以更醒目的颜色显示)
* **设置任务优先级**：
  `!!todo set 1 priority High` (标记为高优先级)
* **添加详细描述**：
  `!!todo set 1 desc 位于主城坐标 100, 64, 200 的 32 堆叠刷铁机`

### 2. 细化任务与依赖管理
* **创建前置子任务**：
  `!!todo add 收集 20 张床` (假设分配 ID 为 `2`)
  `!!todo add 运输 3 名村民` (假设分配 ID 为 `3`)
* **建立依赖关系**：
  `!!todo append 1 dep 2` (主任务 `1` 现在依赖于 `2`)
  `!!todo append 1 dep 3` (主任务 `1` 现在依赖于 `3`)

### 3. 协作与进度记录
* **添加协作者**：
  `!!todo append 1 collab Steve` (邀请 Steve 一起参与)
* **记录进度笔记**：
  `!!todo note 2 已在沙漠村庄收集到 12 张床，还差 8 张`
* **添加标签**：
  `!!todo append 1 label 工业`
