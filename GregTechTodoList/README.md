...
专为《格雷科技》(GregTech) 玩家设计的 MCDReforged 待办事项管理插件。
旨在解决长周期、多线并行的工业流程管理难题，提供极简的输入体验和深度的聊天栏交互。

## ✨ 核心特性

* **极简输入**: 减少繁琐打字，支持字段别称（如 `t` 代表 `tier`），并支持数字映射电压等级（如 `1` -> `LV`）。
* **深度交互**: 充分利用 Minecraft 聊天栏富文本（RText），将聊天栏打造为交互式“工业控制面板”。支持悬浮预览详情、点击执行指令。
* **格雷适配**: 针对 GT 长流程设计，内置电压等级颜色显示，强调任务的前置依赖（Dependencies）管理。
...
| 指令                                  | 别称          | 说明                                                                         |
|:------------------------------------|:------------|:---------------------------------------------------------------------------|
| `!!todo set <id> <prop> <value>`    | `!!todo s`  | 修改单值属性（如 title, tier, priority）。<br>Tier 支持 0-14 数字输入 (0=ULV, ... 14=MAX)。 |
...
* **列表视图**:
    ...
    * `[✎]`: 点击快捷填充笔记指令。
* **详情视图**:
    * **电压等级**: 根据 GT 电压等级显示对应颜色（如 HV 为青色，MV 为金色）。
